...
console.log(
  `using sold channel ${recently_listed_channel} // using listed channel ${recently_sold_channel}`
);
...
async function CreateMessageObject(
  ...
  fungible_amount,
  fungible_tax,
  fungible_address_b16,
  nonfungible_symbol,
  nonfungible_address_b16,
  token_id,
  nonfungible_bps,
  buyer_address_b16,
  seller_address_b16,
  ...
  const zilkroad_url = `https://testnet.zilkroad.io/collections/${toBech32Address(
    nonfungible_address_b16
  )}/${token_id}`;
...
async function SendSoldMessage(messageObject) {
  ...
  const exampleEmbed = new MessageEmbed()
    ...
    .setThumbnail(
      `https://zildexr-testnet.b-cdn.net/${messageObject.nonfungible_address_b16}/${messageObject.token_id}`
    )
    .addFields(
      ...
      {
        name: "Amount",
        value: `${messageObject.fungible_amount} ${messageObject.fungible_symbol}`,
      },
      { name: "Royalty Recipient", value: messageObject.royalty_address_b32 },
      {
        name: "Royalty Amount",
        value: `${messageObject.fungible_tax} ${messageObject.fungible_symbol}`,
      },
...
async function SendListedMessage(messageObject) {
  ...
  const exampleEmbed = new MessageEmbed()
    ...
    .addFields(
      ...
      {
        name: "NFT address",
        value: `${messageObject.nonfungible_address_b32}`,
      },
      {
        name: "NFT Symbol/Token",
        value: `${messageObject.nonfungible_symbol}/${messageObject.token_id}`,
      },
      { name: "TransactionID", value: `${messageObject.tx_url}` },
...
async function getUSDValuefromTokens(ticker, numberOfTokens) {
  ...
  const token_info = await axios.get(
    `https://api.zilstream.com/tokens/${final_fungible}`
  );
